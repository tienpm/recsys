...
class AbstractSampler(object):
    r""":class:`AbstractSampler` is a abstract class, all sampler should inherit from it. This sampler supports returning
    ...
    def set_distribution(self, distribution):
        r"""Set the distribution of sampler.
    ...
    def _uni_sampling(self, sample_num):
        r"""Sample [sample_num] items in the uniform distribution.
    ...
    def _get_candidates_list(self):
        r"""Get sample candidates list for _pop_sampling()
    ...
    def _build_alias_table(self):
        r"""Build alias table for popularity_biased sampling."""
    ...
    def _pop_sampling(self, sample_num):
        r"""Sample [sample_num] items in the popularity-biased distribution.
    ...
    def sampling(self, sample_num):
        r"""Sampling [sample_num] item_ids.
...
class Sampler(AbstractSampler):
    r""":class:`Sampler` is used to sample negative items for each input user. In order to avoid positive items
    ...
    def set_phase(self, phase):
        r"""Get the sampler of corresponding phase.
...
class KGSampler(AbstractSampler):
    r""":class:`KGSampler` is used to sample negative entities in a knowledge graph.
...
class RepeatableSampler(AbstractSampler):
    r""":class:`RepeatableSampler` is used to sample negative items for each input user. The difference from
...
class SeqSampler(AbstractSampler):
    r""":class:`SeqSampler` is used to sample negative item sequence.
    ...
    def sample_neg_sequence(self, pos_sequence):
        r"""For each moment, sampling one item from all the items except the one the user clicked on at that moment.
