...
from reczoo.data.interaction import Interaction
...
class Dataset(torch.utils.data.Dataset):
    ...
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~reczoo.data.dataset.dataset.Dataset`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~reczoo.data.dataset.dataset.Dataset.build`, it will processing dataset into
    DataLoaders, according to :class:`~reczoo.config.eval_setting.EvalSetting`.
    ...
    Attributes:
        ...
        field2type (dict): Dict mapping feature name (str) to its type (:class:`~reczoo.utils.enum_type.FeatureType`).

        field2source (dict): Dict mapping feature name (str) to its source
            (:class:`~reczoo.utils.enum_type.FeatureSource`).
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~reczoo.data.dataset.dataset.Dataset._reset_index` will reset the index of feats.
        """
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        Note:
            ``user_id`` and ``item_id`` has source :obj:`~reczoo.utils.enum_type.FeatureSource.USER_ID` and
            :obj:`~reczoo.utils.enum_type.FeatureSource.ITEM_ID`
        """
    ...
    def _fill_nan(self):
        ...
        For fields with type :obj:`~reczoo.utils.enum_type.FeatureType.TOKEN`, missing value will be filled by
        ``[PAD]``, which indexed as 0.

        For fields with type :obj:`~reczoo.utils.enum_type.FeatureType.FLOAT`, missing value will be filled by
    ...
    def float_like_fields(self):
        """Get fields of type :obj:`~reczoo.utils.enum_type.FeatureType.FLOAT` and
        :obj:`~reczoo.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def token_like_fields(self):
        """Get fields of type :obj:`~reczoo.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~reczoo.utils.enum_type.FeatureType.TOKEN_SEQ`.
    ...
    @property
    def seq_fields(self):
        """Get fields of type :obj:`~reczoo.utils.enum_type.FeatureType.TOKEN_SEQ` and
        :obj:`~reczoo.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def non_seq_fields(self):
        """Get fields of type :obj:`~reczoo.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~reczoo.utils.enum_type.FeatureType.FLOAT`.
    ...
    def build(self):
        ...
        See :class:`~reczoo.config.eval_setting.EvalSetting` for details.
    ...
    def _dataframe_to_interaction(self, data):
        """Convert :class:`pandas.DataFrame` to :class:`~reczoo.data.interaction.Interaction`.

        Args:
            data (pandas.DataFrame): data to be converted.

        Returns:
            :class:`~reczoo.data.interaction.Interaction`: Converted data.
        """
        new_data = {}
        for k in data:
            value = data[k].values
